...
export default function EditHook({ hookId }) {
    ...
    useEffect(() => {
        // Fetch the existing hook data
        fetch(`/api/hooks/${hookId}`)
    ...
    const submit = (e) => {
        e.preventDefault();
        put(`/api/hooks/${hookId}`);
    };

    return (
        <div>
            <Head title="Edit Hook" />

            <h1 className="text-2xl font-bold">Edit Hook</h1>
